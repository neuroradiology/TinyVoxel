...
public class GwtInput implements Input {
	...
	public boolean isButtonPressed (int button) {
		return (button == Buttons.LEFT && touched[0]) || (button == Buttons.RIGHT && touched[1]);
	}
  ...
  // kindly borrowed from our dear playn friends...
	...
	private void handleEvent (NativeEvent e) {
		...
		if (e.getType().equals("mousedown")) {
			if (!e.getEventTarget().equals(canvas) || (touched[0] && e.getButton() == NativeEvent.BUTTON_LEFT) ||
                    (touched[1] && e.getButton() == NativeEvent.BUTTON_RIGHT)) {
			...
			hasFocus = true;
			this.justTouched = !(touched[0] || touched[1]);
		...
		if (e.getType().equals("mouseup")) {
			if ((!touched[0] && e.getButton() == NativeEvent.BUTTON_LEFT)
                    || (!touched[1] && e.getButton() == NativeEvent.BUTTON_RIGHT)) return;
			this.pressedButtons.remove(getButton(e.getButton()));
            this.touched[0] = this.pressedButtons.contains(Buttons.LEFT);
            this.touched[1] = this.pressedButtons.contains(Buttons.RIGHT);
			if (isCursorCatched()) {
				this.deltaX[0] = (int)getMovementXJSNI(e);
				this.deltaY[0] = (int)getMovementYJSNI(e);
				this.touchX[0] += getMovementXJSNI(e);
